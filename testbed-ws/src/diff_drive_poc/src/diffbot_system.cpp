...
#include "diff_drive_poc/diffbot_system.hpp"
...
{
  MotorVelCmdPublisherUtil::MotorVelCmdPublisherUtil() : Node("minimal_publisher")
  {
    RCLCPP_INFO(rclcpp::get_logger("MotorVelCmdPublisherUtil"), "creating util.....");
    this->publisher = this->create_publisher<std_msgs::msg::String>("/driver/motor_vel_cmd", 10);
  }

  void MotorVelCmdPublisherUtil::publish(float cmd_1, float cmd_2)
  {
    RCLCPP_INFO(rclcpp::get_logger("MotorVelCmdPublisherUtil"), ("[ " + std::to_string(cmd_1) + " " + std::to_string(cmd_2) + " ]").c_str());
    auto message = std_msgs::msg::String();
    message.data = std::to_string(cmd_1) + " " + std::to_string(cmd_2);
    this->publisher->publish(message);
  }

  DiffBotSystemHardware::DiffBotSystemHardware() : publisher_util()
  {
    RCLCPP_INFO(rclcpp::get_logger("DiffBotSystemHardware"), "Constructing.....");
  }
  ...
  hardware_interface::return_type diff_drive_poc::DiffBotSystemHardware::write()
  {
    ...
    this->publisher_util.publish(hw_commands_[0], hw_commands_[1]);
...
PLUGINLIB_EXPORT_CLASS(
    diff_drive_poc::DiffBotSystemHardware, hardware_interface::SystemInterface)
